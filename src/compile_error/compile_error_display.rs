...
impl Display for CompileError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let out = colour_format![
            ...
            blue("\n --> "), cyan(&self.position.start().file_name),
            blue(":"), yellow(&ln), blue(":"), yellow(&self.position.start().ln_idx.to_string()),
            blue("\n"), yellow(&ln), blue(" | "), none(&line), // Only works for single line errors, change later if needed
            blue("\n  | "), red(&arrw),
            blue("\n <--"),
        ];
...
fn sample(position: &Position, msg: &str) -> (String, String) {
    ...
    let end_trim = cal_trim(line.len() as u16, position.end().ln_idx);
